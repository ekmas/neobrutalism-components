  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex w-[200px] cursor-pointer items-center rounded-md border-2 border-black bg-main px-7 py-3 font-bold shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all hover:translate-x-[3px] hover:translate-y-[3px] hover:shadow-none"
      >
        ...
        {items.map((item, index) => {
          return (
            <a
              key={index}
              href={item.link}
              className="block w-full border-b-2 border-black bg-main px-7 py-3 no-underline first:rounded-t-[5px] last:rounded-b-[5px] hover:bg-mainAccent"
            >
              {item.name}
            </a>
          )
