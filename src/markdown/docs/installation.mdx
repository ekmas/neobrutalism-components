...
On the styling page you can pick a monochromatic or duotone color palette, border radius, box shadow, font weights and a Google font. The copied styling already includes the `@import` for the chosen font. To use any other Google font, swap the `@import` URL and the `--base-font-family` variable.
